...
import { getAlert, setAlert } from "@/store/slices/appSlice";
const SignUpPage = () => {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      const errorMessage =
        err?.data?.message || "Sign up failed. User already exist.";
      ...
      {isLoading ? (
        ...
        <FormControl className="p-4">
          <VStack space="xl">
            ...
            {/* Sign Up Button */}
            <Button
              className="w-full h-[40px] bg-primary dark:bg-primary"
            ...
            <VStack space="md">
              <Button
                className="w-full h-[40px] bg-background-light dark:bg-surface-dark border border-secondary dark:border-secondary"
                onPress={handleGoogleSignUp}
              >
                <ButtonIcon as={GoogleIcon} />
                <ButtonText className="text-text-base dark:text-text-base">
                  Sign up with Google
                </ButtonText>
              </Button>
            ...
            <HStack className="items-center pt-4 justify-center">
              <Text className="text-text-muted dark:text-text-muted italic">
                Already have an account?
              </Text>
              <TouchableOpacity onPress={handleLoginRedirect}>
                <Text className="ml-2 text-primary dark:text-primary font-semibold">
                  Sign in
                </Text>
              </TouchableOpacity>
            </HStack>
